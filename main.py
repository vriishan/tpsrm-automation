...
while True:
    ...
    if choice == '1':
        # data = {}
        # for k, upguard_vendor in upguard_file.items():
        #     if k in auditboard_file:
        #         auditboard_vendor = auditboard_file[k]
        #         if upguard_vendor['attributes']['TPSP Status'] and upguard_vendor['attributes']['TPSRA Status']:
        #             auditable_entity = {'id': auditboard_vendor['id']}
        #             auditboard_vendor[AB_TPSP_STATUS] = 13 if upguard_vendor['attributes']['TPSP Status'] == 'Complete' else 16
        #             auditable_entity[AB_TPSP_STATUS] = 13 if upguard_vendor['attributes']['TPSP Status'] == 'Complete' else 16

        #             if upguard_vendor['attributes']['TPSRA Status'] == 'Complete':
        #                 auditboard_vendor[AB_TPSRA_STATUS] = 2
        #                 auditable_entity[AB_TPSRA_STATUS] = 2
        #             elif upguard_vendor['attributes']['TPSRA Status'] == 'Submitted':
        #                 auditboard_vendor[AB_TPSRA_STATUS] = 3
        #                 auditable_entity[AB_TPSRA_STATUS] = 3
        #             else:
        #                 auditboard_vendor[AB_TPSRA_STATUS] = 4
        #                 auditable_entity[AB_TPSRA_STATUS] = 4

        #             payload = {"auditable_entity": auditable_entity}
        #             data.append(payload)
                    
        # asyncio.run(run_auditboard_update(data))
        # write_json(auditboard_file, './data/auditboard_data_updated.json')
        # print(f'{len(auditboard_file)} records modified')
        pass
